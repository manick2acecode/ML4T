...
class RTLearner(object):
    ...
    def build_tree(self, data):
        ...
        if data.shape[0] <= self.leaf_size or len(data.shape) == 1:
            ...
            split_value = np.median(data[:, best_i], axis=0)
            if split_value == max(data[:, best_i]):
                return np.array([['leaf', np.mean(data_y), -1, -1]])

            left_tree = self.build_tree(data[data[:, best_i] <= split_value])
            right_tree = self.build_tree(data[data[:, best_i] > split_value])
            root = np.array([[best_i, split_value, 1, left_tree.shape[0] + 1]])
